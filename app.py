...
from ai_tryon import generate_ai_tryon, get_api_key, AITryOnError  # noqa: E402
...
with st.sidebar.expander("✨ הדמיה ריאליסטית (AI)"):
    st.caption(
        "להדמיה ריאליסטית של המסגרת על הפנים (מנוע gpt-image-1 של OpenAI). "
        "דורש מפתח OpenAI API - השירות בתשלום, כמה סנטים לכל הדמיה. "
        "המפתח נשמר רק בזיכרון הדפדפן לזמן השימוש, לא נכתב לדיסק. "
        "אפשר גם להגדיר את המפתח כמשתנה סביבה OPENAI_API_KEY במקום להזין כאן."
    )
...
if results:
    ...
    st.session_state.setdefault("ai_tryon", {})  # frame_id -> נתיב PNG של הדמיית AI
    ...
    with center_col:
        ...
        if show_landmarks:
            ...
            ai_path = st.session_state.ai_tryon.get(frame["id"])
            has_ai = bool(ai_path) and os.path.exists(ai_path)

            if has_ai:
                # הדמיית AI קיימת למסגרת זו - מציגים אותה (מתוך המטמון, בלי חיוב חוזר)
                st.image(ai_path, use_container_width=True)
                st.markdown(
                    f"<div class='ofa-center-caption'>{frame['brand']} {frame['model']} · "
                    f"<span class='pct'>✨ נוצר על-ידי AI</span></div>",
                    unsafe_allow_html=True,
                )
            else:
                ...
                if selected["tryon_path"]:
                    st.markdown(
                        f"<div class='ofa-center-caption'>{frame['brand']} "
                        f"{frame['model']} · "
            ...
            # --- כפתור הדמיה ריאליסטית מבוססת AI (רק אם יש תמונת מסגרת להלביש) ---
            if os.path.exists(frame["image_png"]):
                ai_label = "🔄 צור מחדש (AI)" if has_ai else "✨ הדמיה ריאליסטית (AI)"
                if st.button(ai_label, use_container_width=True, key="ai_tryon_btn"):
                    key = get_api_key(st.session_state.get("openai_key"))
                    if not key:
                        st.warning(
                            "כדי ליצור הדמיה ריאליסטית יש להזין מפתח OpenAI API "
                            "בסרגל הצד (תחת '✨ הדמיה ריאליסטית (AI)')."
                        )
                    else:
                        with st.spinner("יוצר הדמיה ריאליסטית עם AI... (עשוי לקחת כמה שניות)"):
                            try:
                                frame_desc = (
                                    f"{frame.get('brand', '')} {frame.get('model', '')}, "
                                    f"{frame.get('color_name', '')} {frame.get('shape', '')} "
                                    f"{frame.get('material', '')} frame"
                                ).strip()
                                png_bytes = generate_ai_tryon(
                                    results["face_path"], frame["image_png"], key,
                                    frame_desc=frame_desc,
                                )
                                ai_out = f"data/_ai_tryon_{frame['id']}.png"
                                with open(ai_out, "wb") as w:
                                    w.write(png_bytes)
                                st.session_state.ai_tryon[frame["id"]] = ai_out
                                st.rerun()
                            except AITryOnError as e:
                                st.error(str(e))
                if has_ai:
                    st.caption("💡 הדמיה זו נוצרה ע\"י AI לצורך המחשה ואינה מדויקת ב-100%.")
